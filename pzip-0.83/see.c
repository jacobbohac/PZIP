...
See_State* see_Get_State(   See* see,   uint escape_count,   uint total_symbol_count,   u32 key,   const Context* context   ) {
    ...
    {   /****************************/
        ...
        hash2 <<= 2;
        {   Context* p = context->parent;
            if (p)   hash2 |= min( p->followset_size, 3 );
        }
